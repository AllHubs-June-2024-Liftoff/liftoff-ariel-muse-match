...
-- "mysql -u <A> -p <B> < your/current/directory/path/backend/dbdumps/musedatabase_dump.sql"
      ...
      -- the -p option here means it will require your password entered after command is entered
        ...
        -- "B"  here would be what the .sql dump file will be mapped to (Use "musedatabase"
        which is what youve already created in first step).
        Basically its mapping the database structure that trey has already created using hibernate in
        java file which will populate our 'musedatabase' holder with the sql
